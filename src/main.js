...
import { processFrames, framesHaveMatchingDimensions, packFrames, createManifest } from './sprite-processor.js';
...
function previewFrames() {
  ...
  setHidden('clear-files', !selectedFrames().length); updateSourceDimensions(); updateSheetSummary(); updateTrimWarning(); validate();
}

function updateTrimWarning() {
  const mismatchedIndividualFrames = mode() === 'frames' && state.frames.length > 1 && !framesHaveMatchingDimensions(state.frames);
  setHidden('trim-warning', !$('trim').checked || !mismatchedIndividualFrames);
}

function updateSheetSummary() {
  const summary = $('source-summary'); summary.replaceChildren();
  if (!selectedFrames().length) return setHidden('source-summary', true);
  const values = mode() === 'sheet' ? [] : [`${state.frames.length} frame${state.frames.length === 1 ? '' : 's'} naturally sorted by filename`];
...
async function receiveFiles(files) {
  ...
  disposeInput(); if (mode() === 'sheet') clearFrameSuggestion(); const result = await decodeFiles(list, { naturalSort: mode() === 'frames' });
...
form.addEventListener('change', (event) => { if (event.target.name === 'mode') return configureMode(); if (event.target === fileInput || event.target === $('folder-input')) return receiveFiles(event.target.files); invalidate('Options changed. Generate again to update the output.'); setHidden('columns-field', $('layout').value !== 'grid'); updateTrimWarning(); updateSheetSummary(); validate(); });
form.addEventListener('input', (event) => { if (['frame-width','frame-height','target-size','padding','columns','basename'].includes(event.target.id)) { invalidate(); updateSheetSummary(); validate(); } });
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    const processed = processFrames(sourceFrames, options.trim, options.targetSize); const packed = packFrames(processed, options); const basename = sanitizeBasename($('basename').value); $('basename').value = basename;
